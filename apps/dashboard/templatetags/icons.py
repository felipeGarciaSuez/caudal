"""Inline SVG icons (Lucide, ISC license). No emojis in the codebase.

Generated from lucide-static. Use in templates: {% load icons %}{% icon "coffee" %}.
"""
...
# icon name -> inner SVG markup (24x24 viewBox, stroke=currentColor).
ICONS = {
...
def icon(name, cls="icon"):
    """Render an inline SVG icon by name; falls back to a neutral tag icon."""
    inner = ICONS.get(name) or ICONS["tag"]
    return mark_safe(_SVG_OPEN.format(cls=cls) + inner + "</svg>")
